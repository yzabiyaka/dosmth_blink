...
class FetchWorker extends Worker {
  ...
  async consume(fetchMessage) {
    console.dir(fetchMessage, { colors: true, showHidden: true });
    // const message = FetchMessage.fromRabbitMessage(rabbitMessage);
    // message.validate();
    // const processResult = await this.process(message);
  }
