...
class Exchange {
  ...
  publishDirect(queue, payload) {
    // Queues are bound to exchange with their name
    return this.publish(queue.name, payload);
  }

  async publish(routingKey, payload) {
    const message = Exchange.prepareMessage(payload);
    const options = {
    ...
    const result = await this.channel.publish(this.name, routingKey, message, options);

    // Always true.
    return true;
  }

  static prepareMessage(payload) {
