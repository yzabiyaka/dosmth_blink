...
class WebController {
  ...
  sendError(ctx, error) {
    ...
    if (error instanceof ValidationError) {
      ...
      ctx.body.hint = 'See fields property for details'
      ctx.body.fields = error.fields;

      ctx.status = 422;
    }
    this.log(ctx, `Code ${ctx.status} | Error ${error}`);
  }

  log(ctx, message) {
    let fullMessage = `${ctx.request.method} ${ctx.request.originalUrl} | Request ${ctx.id} | ${message}`;
